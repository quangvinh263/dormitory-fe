...
const ContractFilter = ({ filters, setFilters, totalResults }) => {

  const handleClearFilters = () => {
    setFilters({ search: '', status: '', endDate: '' });
  };
  ...
  return (
    <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm mb-6 space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {/* 3. Ngày kết thúc & Nút Xóa */}
        <div className="flex items-end gap-2">
            <div className="flex-1">
                <label className="text-xs font-semibold text-gray-700 mb-1 block">Ngày kết thúc hợp đồng</label>
                <input 
                    type="date" 
            ...
            <button
                ...
                className="p-2.5 mb-px text-red-500 hover:bg-red-50 rounded-lg transition-colors" 
                title="Xóa bộ lọc">
      ...
      <div className="pt-2">
         <p className="text-xs text-gray-500">Tìm thấy <span className="font-bold text-gray-900">{totalResults}</span> hợp đồng phù hợp.</p>
      </div>
    </div>
  );
};
