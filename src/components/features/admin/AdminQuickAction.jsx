      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm h-full flex flex-col">
        ...
        <div className="p-5 space-y-4 overflow-y-auto flex-1">
          {actions.map((item, index) => (
            <div key={index} 
              className="flex items-center justify-between gap-4 p-4 rounded-xl border border-gray-100 hover:border-gray-300 transition-all bg-gray-50/50 hover:bg-white">
              ...
              <button 
                ...
                className={`hidden sm:block text-xs font-bold text-white px-3 py-2 rounded-lg transition-colors ${item.btnColor}`}>
                {item.btnText}
              </button>
            </div>
          ))}
