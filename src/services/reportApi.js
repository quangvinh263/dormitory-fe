...
export const exportStudentContracts = async (studentId) => {
    try {
        const response = await axios.get(`${API_URL}/Report/export/student-contracts/${studentId}`, {
            responseType: 'blob'
        });
        downloadFile(response.data, `BaoCao_HopDong_${studentId}.xlsx`);
        return { success: true };
    } catch (error) {
        return { success: false, message: error.response?.data?.message || 'Error exporting student contracts' };
    }
};
